...
function backtrackDedup(xorb, computeHmac, shardData, chunkCache, chunkMetadata, dedupedBytes) {
  ...
  for (let chunkToRecheckIndex = _nullishCoalesce(_optionalChain([xorb, 'access', _76 => _76.immutableData, 'optionalAccess', _77 => _77.chunkIndex]), () => ( 0)); chunkToRecheckIndex < xorb.chunks.length; chunkToRecheckIndex++) {
  ...
  try {
    ...
    const processChunks = async (chunkData) => {
      for (const chunk2 of chunkData) {
        chunksProcessed++;
        if (_optionalChain([opts, 'optionalAccess', _78 => _78.isAtBeginning]) && chunksProcessed === 1) {
          chunk2.dedup = true;
        }
    ...
    while (true) {
      if (_optionalChain([opts, 'optionalAccess', _79 => _79.end]) !== void 0 && totalBytes >= opts.end) {
        break;
      }
      if (_optionalChain([opts, 'optionalAccess', _80 => _80.maxChunks]) !== void 0 && chunksProcessed >= opts.maxChunks) {
        break;
      }
      const { done, value } = await reader.read();
      if (done) {
      ...
      progressHint: {
        progressCallback: (progress) => {
          for (const file of xorb.files) {
            _optionalChain([params, 'access', _81 => _81.yieldCallback, 'optionalCall', _82 => _82({
              event: "fileProgress",
              path: file.path,
  ...
  try {
    const allOperations = (await Promise.all(
      params.operations.map(async (operation) => {
        ...
        _optionalChain([abortSignal, 'optionalAccess', _83 => _83.throwIfAborted, 'call', _84 => _84()]);
    ...
    const gitAttributes = _optionalChain([allOperations, 'access', _85 => _85.filter, 'call', _86 => _86(isFileOperation), 'access', _87 => _87.find, 'call', _88 => _88((op) => op.path === ".gitattributes"), 'optionalAccess', _89 => _89.content]);
    for (const operations of chunk(allOperations.filter(isFileOperation), 100)) {
      ...
      _optionalChain([abortSignal, 'optionalAccess', _90 => _90.throwIfAborted, 'call', _91 => _91()]);
    ...
    for (const operations of chunk(
      ...
      _optionalChain([abortSignal, 'optionalAccess', _92 => _92.throwIfAborted, 'call', _93 => _93()]);
      const payload = {
        operation: "upload",
      ...
      if (useXet) {
        for (const obj of json.objects) {
          ...
          if (!_optionalChain([obj, 'access', _94 => _94.actions, 'optionalAccess', _95 => _95.upload])) {
            yield {
              event: "fileProgress",
              path: op.path,
              progress: 1,
        ...
        const source = async function* () {
          for (const obj of json.objects) {
            const op = shaToOperation.get(obj.oid);
            if (!op || !_optionalChain([obj, 'access', _96 => _96.actions, 'optionalAccess', _97 => _97.upload])) {
              continue;
            }
            _optionalChain([abortSignal, 'optionalAccess', _98 => _98.throwIfAborted, 'call', _99 => _99()]);
        ...
        yield* eventToGenerator((yieldCallback, returnCallback, rejectCallback) => {
          return promisesQueueStreaming(
            json.objects.map((obj) => async () => {
              ...
              _optionalChain([abortSignal, 'optionalAccess', _100 => _100.throwIfAborted, 'call', _101 => _101()]);
              if (obj.error) {
                const errorMessage = `Error while doing LFS batch call for ${operations[shas.indexOf(obj.oid)].path}: ${obj.error.message}${batchRequestId ? ` - Request ID: ${batchRequestId}` : ""}`;
                throw new HubApiError(res.url, obj.error.code, batchRequestId, errorMessage);
              }
              if (!_optionalChain([obj, 'access', _102 => _102.actions, 'optionalAccess', _103 => _103.upload])) {
                yieldCallback({
                  event: "fileProgress",
                  path: op.path,
                  progress: 1,
              ...
              if (_optionalChain([header, 'optionalAccess', _104 => _104.chunk_size])) {
                ...
                _optionalChain([abortSignal, 'optionalAccess', _107 => _107.throwIfAborted, 'call', _108 => _108()]);
...
async function* listDatasets(params) {
  const accessToken = params && checkCredentials(params);
  let totalToFetch = _nullishCoalesce(_optionalChain([params, 'optionalAccess', _111 => _111.limit]), () => ( Infinity));
  const search = new URLSearchParams([
    ...Object.entries({
      limit: String(Math.min(totalToFetch, 500)),
      ..._optionalChain([params, 'optionalAccess', _112 => _112.search, 'optionalAccess', _113 => _113.owner]) ? { author: params.search.owner } : void 0,
      ..._optionalChain([params, 'optionalAccess', _114 => _114.search, 'optionalAccess', _115 => _115.query]) ? { search: params.search.query } : void 0
    }),
    ..._nullishCoalesce(_optionalChain([params, 'optionalAccess', _116 => _116.search, 'optionalAccess', _117 => _117.tags, 'optionalAccess', _118 => _118.map, 'call', _119 => _119((tag) => ["filter", tag])]), () => ( [])),
    ...DATASET_EXPAND_KEYS.map((val) => ["expand", val]),
    ..._nullishCoalesce(_optionalChain([params, 'optionalAccess', _120 => _120.additionalFields, 'optionalAccess', _121 => _121.map, 'call', _122 => _122((val) => ["expand", val])]), () => ( []))
  ]).toString();
  let url = `${_optionalChain([params, 'optionalAccess', _123 => _123.hubUrl]) || HUB_URL}/api/datasets` + (search ? "?" + search : "");
  while (url) {
    const res = await (_nullishCoalesce(_optionalChain([params, 'optionalAccess', _124 => _124.fetch]), () => ( fetch)))(url, {
      headers: {
    ...
    for (const item of items) {
      yield {
        ..._optionalChain([params, 'optionalAccess', _125 => _125.additionalFields]) && pick(item, params.additionalFields),
        id: item._id,
        name: item.id,
...
async function datasetInfo(params) {
  ...
  const search = new URLSearchParams([
    ...DATASET_EXPAND_KEYS.map((val) => ["expand", val]),
    ..._nullishCoalesce(_optionalChain([params, 'optionalAccess', _126 => _126.additionalFields, 'optionalAccess', _127 => _127.map, 'call', _128 => _128((val) => ["expand", val])]), () => ( []))
  ]).toString();
  const response = await (params.fetch || fetch)(
    `${_optionalChain([params, 'optionalAccess', _129 => _129.hubUrl]) || HUB_URL}/api/datasets/${params.name}/revision/${encodeURIComponent(
  ...
  return {
    ..._optionalChain([params, 'optionalAccess', _130 => _130.additionalFields]) && pick(data, params.additionalFields),
    id: data._id,
    name: data.id,
...
async function fileDownloadInfo(params) {
  ...
  let size;
  let xetInfo;
  if (_optionalChain([resp, 'access', _131 => _131.headers, 'access', _132 => _132.get, 'call', _133 => _133("Content-Type"), 'optionalAccess', _134 => _134.includes, 'call', _135 => _135("application/vnd.xet-fileinfo+json")])) {
  ...
  return {
    ...
    url: resp.url && (new URL(resp.url).origin === new URL(hubUrl).origin || _optionalChain([resp, 'access', _136 => _136.headers, 'access', _137 => _137.get, 'call', _138 => _138("X-Cache"), 'optionalAccess', _139 => _139.endsWith, 'call', _140 => _140(" cloudfront")])) ? resp.url : url
  };
}
...
async function* listModels(params) {
  const accessToken = params && checkCredentials(params);
  let totalToFetch = _nullishCoalesce(_optionalChain([params, 'optionalAccess', _141 => _141.limit]), () => ( Infinity));
  const search = new URLSearchParams([
    ...Object.entries({
      limit: String(Math.min(totalToFetch, 500)),
      ..._optionalChain([params, 'optionalAccess', _142 => _142.search, 'optionalAccess', _143 => _143.owner]) ? { author: params.search.owner } : void 0,
      ..._optionalChain([params, 'optionalAccess', _144 => _144.search, 'optionalAccess', _145 => _145.task]) ? { pipeline_tag: params.search.task } : void 0,
      ..._optionalChain([params, 'optionalAccess', _146 => _146.search, 'optionalAccess', _147 => _147.query]) ? { search: params.search.query } : void 0,
      ..._optionalChain([params, 'optionalAccess', _148 => _148.search, 'optionalAccess', _149 => _149.inferenceProviders]) ? { inference_provider: params.search.inferenceProviders.join(",") } : void 0
    }),
    ..._nullishCoalesce(_optionalChain([params, 'optionalAccess', _150 => _150.search, 'optionalAccess', _151 => _151.tags, 'optionalAccess', _152 => _152.map, 'call', _153 => _153((tag) => ["filter", tag])]), () => ( [])),
    ...MODEL_EXPAND_KEYS.map((val) => ["expand", val]),
    ..._nullishCoalesce(_optionalChain([params, 'optionalAccess', _154 => _154.additionalFields, 'optionalAccess', _155 => _155.map, 'call', _156 => _156((val) => ["expand", val])]), () => ( []))
  ]).toString();
  let url = `${_optionalChain([params, 'optionalAccess', _157 => _157.hubUrl]) || HUB_URL}/api/models?${search}`;
  while (url) {
    const res = await (_nullishCoalesce(_optionalChain([params, 'optionalAccess', _158 => _158.fetch]), () => ( fetch)))(url, {
      headers: {
        accept: "application/json",
        ...accessToken ? { Authorization: `Bearer ${accessToken}` } : void 0
      }
    ...
    for (const item of items) {
      ...
      if (_optionalChain([params, 'optionalAccess', _159 => _159.additionalFields, 'optionalAccess', _160 => _160.includes, 'call', _161 => _161("inferenceProviderMapping")]) && item.inferenceProviderMapping) {
      ...
      yield {
        ..._optionalChain([params, 'optionalAccess', _162 => _162.additionalFields]) && pick(normalizedItem, params.additionalFields),
        id: item._id,
        name: item.id,
        private: item.private,
...
async function* listSpaces(params) {
  ...
  const search = new URLSearchParams([
    ...Object.entries({
      limit: "500",
      ..._optionalChain([params, 'optionalAccess', _163 => _163.search, 'optionalAccess', _164 => _164.owner]) ? { author: params.search.owner } : void 0,
      ..._optionalChain([params, 'optionalAccess', _165 => _165.search, 'optionalAccess', _166 => _166.query]) ? { search: params.search.query } : void 0
    }),
    ..._nullishCoalesce(_optionalChain([params, 'optionalAccess', _167 => _167.search, 'optionalAccess', _168 => _168.tags, 'optionalAccess', _169 => _169.map, 'call', _170 => _170((tag) => ["filter", tag])]), () => ( [])),
    ...[...SPACE_EXPAND_KEYS, ..._nullishCoalesce(_optionalChain([params, 'optionalAccess', _171 => _171.additionalFields]), () => ( []))].map(
      (val) => ["expand", val]
    )
  ]).toString();
  let url = `${_optionalChain([params, 'optionalAccess', _172 => _172.hubUrl]) || HUB_URL}/api/spaces?${search}`;
  while (url) {
    const res = await (_nullishCoalesce(_optionalChain([params, 'optionalAccess', _173 => _173.fetch]), () => ( fetch)))(url, {
      headers: {
        accept: "application/json",
        ...accessToken ? { Authorization: `Bearer ${accessToken}` } : void 0
      }
    ...
    for (const item of items) {
      yield {
        ..._optionalChain([params, 'optionalAccess', _174 => _174.additionalFields]) && pick(item, params.additionalFields),
        id: item._id,
        name: item.id,
        sdk: item.sdk,
        likes: item.likes,
...
async function* listCollections(params) {
  ...
  let totalToFetch = _nullishCoalesce(_optionalChain([params, 'optionalAccess', _175 => _175.limit]), () => ( Infinity));
  searchParams.append("limit", String(Math.min(totalToFetch, 100)));
  if (_optionalChain([params, 'optionalAccess', _176 => _176.sort])) {
    searchParams.append("sort", params.sort);
  }
  if (_optionalChain([params, 'optionalAccess', _177 => _177.search, 'optionalAccess', _178 => _178.owner])) {
    for (const owner of params.search.owner) {
      searchParams.append("owner", owner);
    }
  }
  if (_optionalChain([params, 'optionalAccess', _179 => _179.search, 'optionalAccess', _180 => _180.item])) {
    for (const item of params.search.item) {
      searchParams.append("item", item);
    }
  }
  if (_optionalChain([params, 'optionalAccess', _181 => _181.search, 'optionalAccess', _182 => _182.q])) {
    searchParams.append("q", params.search.q);
  }
  let url = `${_optionalChain([params, 'optionalAccess', _183 => _183.hubUrl]) || HUB_URL}/api/collections?${searchParams}`;
  while (url) {
    const res = await (_nullishCoalesce(_optionalChain([params, 'optionalAccess', _184 => _184.fetch]), () => ( fetch)))(url, {
      headers: {
        accept: "application/json",
        ...accessToken ? { Authorization: `Bearer ${accessToken}` } : void 0
      }
...
async function modelInfo(params) {
  ...
  const search = new URLSearchParams([
    ...MODEL_EXPAND_KEYS.map((val) => ["expand", val]),
    ..._nullishCoalesce(_optionalChain([params, 'optionalAccess', _185 => _185.additionalFields, 'optionalAccess', _186 => _186.map, 'call', _187 => _187((val) => ["expand", val])]), () => ( []))
  ]).toString();
  const response = await (params.fetch || fetch)(
    `${_optionalChain([params, 'optionalAccess', _188 => _188.hubUrl]) || HUB_URL}/api/models/${params.name}/revision/${encodeURIComponent(
      _nullishCoalesce(params.revision, () => ( "HEAD"))
    )}?${search.toString()}`,
    {
      headers: {
  ...
  if (_optionalChain([params, 'optionalAccess', _189 => _189.additionalFields, 'optionalAccess', _190 => _190.includes, 'call', _191 => _191("inferenceProviderMapping")]) && data.inferenceProviderMapping) {
    normalizedData.inferenceProviderMapping = normalizeInferenceProviderMapping(data.id, data.inferenceProviderMapping);
  }
  return {
    ..._optionalChain([params, 'optionalAccess', _192 => _192.additionalFields]) && pick(normalizedData, params.additionalFields),
    id: data._id,
    name: data.id,
    private: data.private,
...
async function oauthHandleRedirect(opts) {
  if (typeof window === "undefined" && !_optionalChain([opts, 'optionalAccess', _193 => _193.redirectedUrl])) {
    throw new Error("oauthHandleRedirect is only available in the browser, unless you provide redirectedUrl");
  }
  if (typeof localStorage === "undefined" && (!_optionalChain([opts, 'optionalAccess', _194 => _194.nonce]) || !_optionalChain([opts, 'optionalAccess', _195 => _195.codeVerifier]))) {
    throw new Error(
      "oauthHandleRedirect requires localStorage to be available, unless you provide nonce and codeVerifier"
    );
  }
  const redirectedUrl = _nullishCoalesce(_optionalChain([opts, 'optionalAccess', _196 => _196.redirectedUrl]), () => ( window.location.href));
  const searchParams = (() => {
    try {
  ...
  const nonce = _nullishCoalesce(_optionalChain([opts, 'optionalAccess', _197 => _197.nonce]), () => ( localStorage.getItem("huggingface.co:oauth:nonce")));
  if (!code) {
  ...
  const codeVerifier = _nullishCoalesce(_optionalChain([opts, 'optionalAccess', _198 => _198.codeVerifier]), () => ( localStorage.getItem("huggingface.co:oauth:code_verifier")));
  if (!codeVerifier) {
  ...
  const hubUrl = _optionalChain([opts, 'optionalAccess', _199 => _199.hubUrl]) || HUB_URL;
  ...
  if (!_optionalChain([opts, 'optionalAccess', _200 => _200.codeVerifier])) {
    localStorage.removeItem("huggingface.co:oauth:code_verifier");
  }
  if (!_optionalChain([opts, 'optionalAccess', _201 => _201.nonce])) {
...
async function oauthHandleRedirectIfPresent(opts) {
  if (typeof window === "undefined" && !_optionalChain([opts, 'optionalAccess', _202 => _202.redirectedUrl])) {
    throw new Error("oauthHandleRedirect is only available in the browser, unless you provide redirectedUrl");
  }
  if (typeof localStorage === "undefined" && (!_optionalChain([opts, 'optionalAccess', _203 => _203.nonce]) || !_optionalChain([opts, 'optionalAccess', _204 => _204.codeVerifier]))) {
    throw new Error(
      "oauthHandleRedirect requires localStorage to be available, unless you provide nonce and codeVerifier"
    );
  }
  const searchParams = new URLSearchParams(_nullishCoalesce(_optionalChain([opts, 'optionalAccess', _205 => _205.redirectedUrl]), () => ( window.location.search)));
...
async function oauthLoginUrl(opts) {
  if (typeof window === "undefined" && (!_optionalChain([opts, 'optionalAccess', _206 => _206.redirectUrl]) || !_optionalChain([opts, 'optionalAccess', _207 => _207.clientId]))) {
    throw new Error("oauthLogin is only available in the browser, unless you provide clientId and redirectUrl");
  }
  if (typeof localStorage === "undefined" && !_optionalChain([opts, 'optionalAccess', _208 => _208.localStorage])) {
    throw new Error(
  ...
  const hubUrl = _optionalChain([opts, 'optionalAccess', _209 => _209.hubUrl]) || HUB_URL;
  const openidConfigUrl = `${new URL(hubUrl).origin}/.well-known/openid-configuration`;
  const openidConfigRes = await fetch(openidConfigUrl, {
    headers: {
      Accept: "application/json"
  ...
  if (_optionalChain([opts, 'optionalAccess', _210 => _210.localStorage])) {
  ...
  const redirectUri = _optionalChain([opts, 'optionalAccess', _211 => _211.redirectUrl]) || (typeof window !== "undefined" ? window.location.href : void 0);
  if (!redirectUri) {
  ...
  const state = JSON.stringify({
    nonce: newNonce,
    redirectUri,
    state: _optionalChain([opts, 'optionalAccess', _212 => _212.state])
  });
  const variables = (
    // @ts-expect-error window.huggingface is defined inside static Spaces.
    typeof window !== "undefined" ? _nullishCoalesce(_optionalChain([window, 'access', _213 => _213.huggingface, 'optionalAccess', _214 => _214.variables]), () => ( null)) : null
  );
  const clientId = _optionalChain([opts, 'optionalAccess', _215 => _215.clientId]) || _optionalChain([variables, 'optionalAccess', _216 => _216.OAUTH_CLIENT_ID]);
  if (!clientId) {
    if (variables) {
  ...
  return `${opendidConfig.authorization_endpoint}?${new URLSearchParams({
    client_id: clientId,
    scope: _optionalChain([opts, 'optionalAccess', _217 => _217.scopes]) || _optionalChain([variables, 'optionalAccess', _218 => _218.OAUTH_SCOPES]) || "openid profile",
    response_type: "code",
...
async function parseSafetensorsMetadata(params) {
  ...
  const quantConfig = _optionalChain([modelConfig, 'optionalAccess', _219 => _219.quantization_config]);
  if (params.path && RE_SAFETENSORS_FILE.test(params.path) || await fileExists({ ...params, path: SAFETENSORS_FILE })) {
    ...
    return {
      ...
      ...params.computeParametersCount ? {
        ...
        parameterTotal: (
          /// shortcut: get param count directly from metadata
          _optionalChain([index, 'access', _220 => _220.metadata, 'optionalAccess', _221 => _221.total_parameters]) ? typeof index.metadata.total_parameters === "number" ? index.metadata.total_parameters : typeof index.metadata.total_parameters === "string" ? parseInt(index.metadata.total_parameters) : void 0 : void 0
        )
      } : void 0
    };
  } else {
...
function getQuantizationMultiplier(tensorName, dtype, quantConfig) {
  ...
  const quantMethod = _optionalChain([quantConfig, 'access', _222 => _222.quant_method, 'optionalAccess', _223 => _223.toLowerCase, 'call', _224 => _224()]);
  switch (quantMethod) {
    case "mxfp4":
...
function shouldSkipTensor(tensorName, quantConfig) {
  ...
  const quantMethod = _optionalChain([quantConfig, 'access', _225 => _225.quant_method, 'optionalAccess', _226 => _226.toLowerCase, 'call', _227 => _227()]);
...
async function spaceInfo(params) {
  ...
  const search = new URLSearchParams([
    ...
    ..._nullishCoalesce(_optionalChain([params, 'optionalAccess', _228 => _228.additionalFields, 'optionalAccess', _229 => _229.map, 'call', _230 => _230((val) => ["expand", val])]), () => ( []))
  ]).toString();
  const response = await (params.fetch || fetch)(
    `${_optionalChain([params, 'optionalAccess', _231 => _231.hubUrl]) || HUB_URL}/api/spaces/${params.name}/revision/${encodeURIComponent(
      _nullishCoalesce(params.revision, () => ( "HEAD"))
    )}?${search.toString()}`,
    {
      headers: {
  ...
  const data = await response.json();
  return {
    ..._optionalChain([params, 'optionalAccess', _232 => _232.additionalFields]) && pick(data, params.additionalFields),
    id: data._id,
    name: data.id,
    sdk: data.sdk,
    likes: data.likes,
...
async function* uploadFilesWithProgress(params) {
  return yield* commitIter({
    ...
    fetch: async (input, init) => {
      ...
      _optionalChain([init, 'access', _233 => _233.signal, 'optionalAccess', _234 => _234.throwIfAborted, 'call', _235 => _235()]);
      xhr.send(init.body);
      return new Promise((resolve2, reject) => {
        ...
        if (init.signal) {
          init.signal.addEventListener("abort", () => {
            xhr.abort();
            try {
              _optionalChain([init, 'access', _236 => _236.signal, 'optionalAccess', _237 => _237.throwIfAborted, 'call', _238 => _238()]);
            } catch (err) {
              reject(err);
            }
          });
...
async function whoAmI(params) {
  ...
  if (typeof _optionalChain([response, 'access', _239 => _239.auth, 'access', _240 => _240.accessToken, 'optionalAccess', _241 => _241.createdAt]) === "string") {
